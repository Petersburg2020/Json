...
public abstract class Document<D extends Document> implements
    ...
    protected void encode() {
        ...
        switch (getType()) {
            ...
            case Journal:
                ...
                break;
            case Magazine:
                tag = Magazine.PIS_TAG;
                break;
            case NewsPaper:
                tag = NewsPaper.PIS_TAG;
                break;
            case Thesis:
                tag = Thesis.PIS_TAG;
        }
    ...
    protected void decode() {
        if (decodedPis != null && decodedPis.isNotEmpty()) {
            switch (decodedPis.getTag()) {
                case Book.PIS_TAG:
                    type = Type.Book;
                    break;
                case Journal.PIS_TAG:
                    type = Type.Journal;
                    break;
                case Magazine.PIS_TAG:
                    type = Type.Magazine;
                    break;
                case NewsPaper.PIS_TAG:
                    type = Type.NewsPaper;
                    break;
                case Thesis.PIS_TAG:
                    type = Type.Thesis;
            ...
            if (decodedJson.containsKey(JSON_DOC_TYPE)) {
                ...
                switch (decodedJson.getString(JSON_DOC_TYPE)) {
                    case Book.JSON_TAG:
                        type = Type.Book;
                        break;
                    case Journal.JSON_TAG:
                        type = Type.Journal;
                        break;
                    case Magazine.JSON_TAG:
                        type = Type.Magazine;
                        break;
                    case NewsPaper.JSON_TAG:
                        type = Type.NewsPaper;
                        break;
                    case Thesis.JSON_TAG:
                        type = Type.Thesis;
                }
    ...
    @Override
    public <A extends Page> D setPages(Pages<A> pages) {
        return pages != null ? setPages(pages.list()) : (D) this;
    }

    @Override
    public <A extends Page> D setPages(List<A> pages) {
        this.pages = new ArrayList<>();
        return addPages(pages);
    }

    @Override
    public <A extends Page> D addPages(A... pages) {
        return addPages(Arrays.asList(pages));
    }

    @Override
    public <A extends Page> D addPages(Pages<A> pages) {
        return pages != null ? addPages(pages.list()) : (D) this;
    }

    @Override
    public <A extends Page> D addPages(List<A> pages) {
        if (pages != null)
            for (A page : pages)
    ...
    @Override
    public <A extends Page> D removePages(A... pages) {
        if (pages != null)
    ...
    @Override
    public <A extends Page> D removePages(List<A> pages) {
        if (pages != null)
            for (Page page : pages)
                if (page != null)
    ...
    @Override
    public Pages<nx.peter.java.document.reader.Page> getPages(int startPage, int endPage) {
        List<nx.peter.java.document.reader.Page> pages = new ArrayList<>();
        if (startPage < endPage)
            for (Page.Creator page : this.pages)
                if (page.getNumber() >= startPage && page.getNumber() <= endPage)
                    pages.add(page);
        return new IPages<>(pages);
    }

    @Override
    public Pages<nx.peter.java.document.reader.Page> generatePages() {
    ...
    @Override
    public Pages<nx.peter.java.document.reader.Page> getPages() {
        List<nx.peter.java.document.reader.Page> pages = new ArrayList<>();
        for (nx.peter.java.document.reader.Page page : this.pages)
            pages.add(page);
        return new IPages<>(pages);
    }
    ...
    protected static class IPages<P extends Page> implements Pages<P> {
        protected List<P> pages;

        public IPages(List<P> pages) {
            this.pages = pages;
        }

        @Override
        public P getPage(int page) {
            return page > 0 && page < size() ? pages.get(page - 1) : null;
        }
        ...
        @Override
        public <A extends Page> int indexOf(A page) {
            int index = 0;
            if (page != null)
                for (P p : pages) {
                    index++;
        ...
        @Override
        public <A extends Page> boolean contains(A page) {
            for (P p : pages)
                if (p.equals(page))
                    return true;
            return false;
        ...
        @Override
        public <A extends Page> boolean equals(Pages<A> another) {
            return another.equals(pages);
        }

        @Override
        public <A extends Page> boolean equals(List<A> pages) {
            return pages != null && pages.equals(this.pages);
        }

        @Override
        public nx.peter.java.json.reader.JsonArray<?> toJson() {
            JsonArray pages = new JsonArray();
            for (P page : this.pages)
                pages.add(((nx.peter.java.document.reader.Page<?, ?, ?>) page).toJson());
            return pages;
        }

        @Override
        public Node<?, ?> toPis() {
            List<nx.peter.java.pis.core.Node> nodes = new ArrayList<>();
            for (P page : pages)
                nodes.add(((nx.peter.java.document.reader.Page) page).toPis());
