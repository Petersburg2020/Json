...
public interface List<I, L extends List, B extends Body> extends Element<L, B> {
    ...
    class Creator<I> extends
        ...
        public Creator<I> fromJson(JsonObject json) {
                    ...
                    for (int n = 0; n < arr.size(); n++) {
                        ...
                        if (o instanceof Map) {
                            ...
                            if (encoding != null) {
                                I element;
                                switch (encoding) {
                                    case "Heading": element = (I) new Heading.Creator().fromJson(obj); break;
                                    case "Image": element = (I) new Image.Creator().fromJson(obj); break;
                                    case "List": element = (I) new Creator<>().fromJson(obj); break;
                                    case "Text": element = (I) new Text.Creator().fromJson(obj); break;
            ...
            if (json != null && json.containsKey(JSON_BODY))
                        ...
                        if (item.isSingleValue())
                            add((I) item.get());
                                ...
                                if (encoding != null) {
                                    I element;
                                    switch (encoding) {
                                        case "Heading": element = (I) new Heading.Creator().fromPis(item); break;
                                        case "Image": element = (I) new Image.Creator().fromPis(item); break;
                                        case "List": element = (I) new Creator<>().fromPis(item); break;
                                        case "Text": element = (I) new Text.Creator().fromPis(item); break;
                                        default: element = null;
                                    }
                                    add(element);
                                }
        ...
        @Override
        public Creator<I> set(List list) throws NotItemException {
